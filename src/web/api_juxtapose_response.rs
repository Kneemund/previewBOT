...
impl APIJuxtaposeResponse {
    ...
    pub(crate) async fn redis_cache_set(
        ...
        connection.hset_multiple(key, &data).await.map_err(|err| {
            println!("Error while setting cached data: {:?}", err);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
    ...
    pub(crate) async fn redis_cache_get_data(
        ...
        connection.hgetall(key).await.ok().and_then(|cached_urls| {
            match (
    ...
    pub(crate) async fn redis_cache_get_expire(
        ...
        match connection.expire_time(key).await {
            Ok(reply) => match reply {
                redis::IntegerReplyOrNoOp::IntegerReply(expire_time) => Ok(expire_time),
                _ => {
                    println!("Expire time not found for key {}.", key);
                    Err(StatusCode::INTERNAL_SERVER_ERROR)
                }
            },
            Err(err) => {
                println!("Error while getting expire time for key {}: {:?}", key, err);
                Err(StatusCode::INTERNAL_SERVER_ERROR)
            }
        }
    }
